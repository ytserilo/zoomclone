...
class VideoComponent extends React.Component{
  constructor(props){
    ...
    this.state = {"video": old_obj.video, "audio": old_obj.audio};
  ...
  video_stream_control(e){
    ...
    let send_data = {"type": "video-stream-control", "conn-id": self.props.component_id};
  ...
  render(){
    ...
    if(this.props.component_id == "self"){
      audio = <span style={{display: "none"}}></span>;
    }
    else{
      audio = <audio autoplay="autoplay" style={{display: "none"}}></audio>;
    }
    ...
    if(state["video"] == true){
      ...
      video_obj = <img src="/static/icons/white-video-solid.svg" />;
    }
    else{
      display_style = {"display": ""};
      video_obj = <img src="/static/icons/white-video-slash-solid.svg" />;
    }

    if(state["audio"] == true){
      microphone_obj = <img src="/static/icons/white-microphone-solid.svg" />;
    }else{
      microphone_obj = <img src="/static/icons/white-microphone-slash-solid.svg" />;
    }
    let img_style = {"display": ""};
    if(state["mini"] == true){
      img_style = {"display": "none"};
    }

    return (
      <div dataId={this.props.component_id} class="user-block">
        <div class="media-view">
          <video videoId={this.props.component_id} onClick={this.props.click_handler} autoplay="autoplay" playsinline="true"></video>
          {audio}
          <div style={display_style} class="user-data">
            <img style={img_style} src={this.props.photo_url} />
...
class BigView extends React.Component{
  constructor(props){
    ...
    this.state = {"x-scroll": 0, "full_screen": "Open"};
  }
  ...
  onwheel_event(e){
    ...
    this.setState({"x-scroll": scroll_x});
    header.scrollTo(scroll_x, 0);
  }

  full_screen(){
    if(this.state.full_screen == "Open"){
      this.setState({"full_screen": "Hide"});
    }
    else{
      this.setState({"full_screen": "Open"});
    }
  ...
  render(){
    return <div style={{display: "contents"}}>
      <div class="header" onWheel={this.onwheel_event}>
        {
          this.props.data["videoblocks"].map(function(item){return item["obj"]})
        }
      </div>

      <div class="big-video">
        <div class="controls">
          <button type="button" onClick={this.props.close_handler} name="button">Close</button>
          <button type="button" onClick={this.full_screen} name="button">{this.state.full_screen}</button>
        </div>

        {this.props.data["main-block"]["obj"]}
      </div>
    </div>
  }
}
...
class MessageObj extends React.Component{
  ...
  download_file(){
    ...
    download(this.props.file.link, conn_id, this.props.file.size, this.props.file.file_name);
  ...
  render(){
    ...
    if(this.props.owner != "self"){
      button = <button type="button" onClick={this.download_file} data-id={this.props.file.link} name="button">
        <img src="/static/icons/download-solid.svg" alt="" />
      </button>;
    }
    else{
      button = <span></span>;
    ...
    if(this.props.file != false){
      ...
      obj = <div class="file">
        <span class="author-name">{this.props.file.sender_name}</span>
      ...
      obj = <div class="text">
        <span class="author-name">{this.props.message.sender_name}</span>
      ...
      <div class="chat-content">
        {
          this.state.messages.map(function(item){
            return <MessageObj file={item.file} owner={item.owner} message={item.message} />;
          })
        }
      </div>
    )
...
class VideoGrid extends React.Component{
  ...
  outputsize(video_block, return_value=false){
    ...
    let new_state = {"current-page": 1, "part-size": part_size, "count-pages": count_pages};
  ...
  init(){
    ...
    document.addEventListener("new-media-connection", async function(e){
      ...
      const myComponent = [true].map((item) => {
        ...
        if(video_component_states[data["id"]] != undefined){
          ...
          video_component_states[data["id"]] = {"video": true, "audio": true};
        }
      ...
      for(let i = 0; i < video_blocks.length; i++){
        if(video_blocks[i]["id"] == data["id"] && data["id"] == "self"){
          ...
          self.setState({"videoblocks": video_blocks});
          await promise;


          video_blocks = [{"id": data["id"], "obj": myComponent[0]}].concat(self.state["videoblocks"]);

          self.setState({"videoblocks": video_blocks});
          promise = new Promise((resolve, reject) => {
            self.resolve = resolve;
          });
          self.setState({"videoblocks": []});
          await promise;
          self.setState({"videoblocks": video_blocks})
          return;
        }
      ...
      if(self.state["main-block"]["id"] == data["id"]){
        ...
        video_blocks.push({"id": data["id"], "obj": myComponent[0]});
      }
      ...
      self.setState({"videoblocks": [], "main-block": false});
      await promise;
      self.setState(newstate);
    });
    ...
    document.addEventListener("close-media-connection", async function(e){
      ...
      if(self.state["main-block"] != false){
        if(self.state["main-block"]["id"] == data["id"]){
          self.setState({"main-block": false});
          return;
        }
      }
  ...
  async click_handler(e){
    ...
    for(let i = 0; i < videoblocks.length; i++){
      if(videoblocks[i]["id"] == id){
        ...
        this.setState({"videoblocks": videoblocks, "main-block": obj});
        break;
      }
    }
  }
  ...
  async close_handler(){
    let ready_resolve;
    let ready = new Promise((resolve, reject) => {ready_resolve = resolve})
    ...
    this.setState({"videoblocks": videoblocks, "main-block": false});
    await promise;
  ...
  left_click_handler(){
    if(this.state["current-page"] - 1 >= 1){
      this.setState({"current-page": this.state["current-page"] - 1});
    }
  }
  ...
  render(){
    ...
    let left_style;
    let right_style;
    let obj_style = {"height": "fit-content"};
    let pags = [];
    ...
    if(this.state["main-block"] == false){
      ...
      if(this.state["part-size"] == "full"){
        ...
        component = videoblocks.map(function(item){return item["obj"]});
      }
      else{
        let i = 0;
        ...
        component = videoblocks.map(function(item){
          let obj;
          item["obj"]
          // /this.props.handler.addEventListener("video-stream-control"
          if(i >= start && i < fin){
            obj = <div style={{display: ""}}>{item["obj"]}</div>;
          }
          else{
            ...
            obj = <div style={{display: "none"}}>{item["obj"]}</div>;
          }
          i++;
          return obj;
        })
      ...
      if(this.state["count-pages"] > 1){
        for(let i = 0; i < this.state["count-pages"]; i++){
          if(i == this.state["current-page"] - 1){
            pags.push(<span class="active-dot"></span>);
          }
          else{
            pags.push(<span></span>);
          }
      ...
      component = <BigView data={this.state} close_handler={this.close_handler} full_screen={this.full_screen}/>;
    }

    return <div style={obj_style} class="video-wrapper">
      <button type="button" onClick={this.left_click_handler} style={{display: left_style}} id="left">
        <img src="/static/icons/angle-left-solid.svg" />
      </button>
      <button type="button" onClick={this.right_click_handler} style={{display: right_style}} id="right">
        <img src="/static/icons/angle-right-solid.svg" />
      </button>
      <div class="page-dots">
        {pags}
      </div>
...
ReactDOM.render(
 <VideoGrid />,
 document.querySelector(".video-block")
)
...
class MessageComponent extends React.Component{
  ...
  send_message(e){
    ...
    let message_text = this.message_text.current.value;

    // delete files from form
  ...
  render(){
    return <div class="message-input-block">
      <div class="info-block">
        <textarea ref={this.message_text} cols="23" onInput={this.input_handler}></textarea>
        <div style={{width: "2.5rem", position: "relative"}}>
          <input style={{display: "none"}} type="file" multiple onChange={this.choose_files} ref={this.file_obj}/>
          <button type="button" onClick={this.open_file_menu}>
            <img src="/static/icons/paperclip-solid.svg" alt="files" />
          </button>
        </div>

      </div>
      <div style={{width: "4rem", position: "relative"}}>
      <button id="send" onClick={this.send_message} type="button">
        <img src="/static/icons/paper-plane-solid.svg" alt="send" />
      </button>
      </div>
...
ReactDOM.render(
  <MessageComponent />,
  document.querySelector("#message-input-block-wrapper")
);

class RecordScreen extends React.Component{
  constructor(props){
    super(props);
    // props -> {parent_obj, screen_stream}
    let recorder = new MediaRecorder(this.props.screen_stream);
    recorder.start();

    const self = this;
    this.props.bridge.dispatchEvent(new CustomEvent("child-obj", {detail: self}));
    ...
    this.state = {"recorder": recorder, "status": "recording"};
  }
  ...
  render(){
    ...
    if(status == "recording"){
      buttons_arr = [
        <button type="button" onClick={this.stop_recording} name="button">Stop</button>,
        <button type="button" onClick={this.pause_recording} name="button">Pause</button>
      ];
    }
    else if(status == "pause"){
      buttons_arr = [
        <button type="button" onClick={this.stop_recording} name="button">Stop</button>,
        <button type="button" onClick={this.resume_recording} name="button">Resume</button>
      ];
    }

    return (
      <div class="manage-stream-recorder">
        {
...
class ManageComponent extends React.Component{
    ...
    this.change_size = function(){
      ...
      if(main_container.offsetWidth < 700){
        if(self.state["mobile-view"] == false){
          ...
          self.setState({"mobile-view": true, "comment": false});
        }
      }
      else{
        let obj = document.querySelector("#comment");
        obj.children[0].setAttribute("src", "/static/icons/comment-solid.svg");
        self.setState({"mobile-view": false, "comment": true});
      }
    }
  ...
  toggle(e, obj_name){
    ...
    if(this.state[obj_name] == true){
      ...
      let new_state = {}; new_state[obj_name] = false;
      this.setState(new_state);
    }
    else{
      child.setAttribute("src", "/static/icons/{}-solid.svg".replace("{}", obj_name));

      let new_state = {}; new_state[obj_name] = true;
      this.setState(new_state);
    }
  }
  ...
  microphone_toggle(e){
    if(media.tracks == undefined || media.tracks.audio == undefined){
      ...
      let new_state = {}; new_state["microphone"] = false;
  ...
  video_toggle(e){
    if(media.tracks == undefined || media.tracks.video == undefined){
      ...
      let new_state = {}; new_state["video"] = false;
  ...
  share_screen(e){
    ...
    if(this.state["share-screen"] == false){
      navigator.mediaDevices.getDisplayMedia({"video": true, "audio": true}).then(async function(stream){
        ...
        self.setState({"share-screen": stream});
      }).catch(function(err){
        // send stop event-loop continue stream video
        self.setState({"share-screen": false});
      });
  ...
  stop_recording(from_child=true){
    ...
    this.setState({"record-screen": false});
  }
  ...
  screen_record(e){
    ...
    if(this.state["record-screen"] == false){
      navigator.mediaDevices.getDisplayMedia({"video": true, "audio": true}).then(function(stream){
        ...
        function handler(e){
          self.setState({"record-screen": e.detail});
          bridge_obj.removeEventListener("child-obj", handler);
        }
        bridge_obj.addEventListener("child-obj", handler);

        ReactDOM.render(
          <RecordScreen parent_obj={self} bridge={bridge_obj} screen_stream={stream} />,
        ...
        stream.oninactive = function(e){
          ...
          self.setState({"record-screen": false});
        }
  ...
  render(){
    ...
    return (
      <div class="settings-wrapper">
        <button type="button" id="mic" onClick={this.microphone_toggle} name="button">
          <img src="/static/icons/microphone-solid.svg"/>
        </button>
        <button type="button" id="cam" onClick={this.video_toggle} name="button">
          <img src="/static/icons/video-solid.svg"/>
        </button>
        <button type="button" class={share_screen_class} onClick={this.share_screen} name="button">
          <img src="/static/icons/desktop-solid.svg"/>
        </button>
        <button type="button" onClick={this.settings} name="button">
          <img src="/static/icons/cog-solid.svg"/>
        </button>
        <button id="comment" type="button" onClick={this.chat_toggle} name="button">
          <img src="/static/icons/comment-solid.svg"/>
        </button>
        <button type="button" class={record_screen_class} onClick={this.screen_record} name="button">
          <img src="/static/icons/record-vinyl-solid.svg"/>
        </button>
        <button type="button" style={{display: "none"}} id="offer_view" name="button">
          <span id="offer-counter"></span>
          <img src="/static/icons/sign-in-alt-solid.svg"/>
        </button>
      </div>
    );
  }
